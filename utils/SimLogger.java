...
public class SimLogger {
	...
	public void simStarted(String outFolder, String fileName) {
		...
		filePrefix = fileName;                    // Log文件名
		outputFolder = outFolder;				  // 输出文件路径
		taskMap = new HashMap<Integer, LogItem>();  // Hash任务图
		vmLoadList = new LinkedList<VmLoadLogItem>(); // 虚拟机加载列表
		apDelayList = new LinkedList<ApDelayLogItem>(); // 延迟列表
		
		numOfAppTypes = SimSettings.getInstance().getTaskLookUpTable().length; // 任务长度
		// 深度文件记录开关 ？
	...
	public void setFID(int taskId,int FID){
		taskMap.get(taskId).setFId(FID);
	}
	...
	private void recordLog(int taskId){
		LogItem value = taskMap.remove(taskId);    //获取任务，并将其从列表删除

        if(!taskFlag.contains(value.getFId())){
			taskFlag.add(value.getFId());
            return;
        }

		if (value.isInWarmUpPeriod())              //若它处于预处理期就结束了则不管
			return;
...
class LogItem {
	...
	public void taskEnded(double time) {
		taskEndTime = time;
		status = SimLogger.TASK_STATUS.COMLETED;  //任务完成标签
	}
